...
resource "helm_release" "sync" {
  ...
  set = [
    { name = "gitRepository.spec.url", value = var.repository_url },
    { name = "gitRepository.spec.interval", value = "5m" },
    { name = "gitRepository.spec.ref.branch", value = "main" },
    { name = "gitRepository.spec.recurseSubmodules", value = true },
